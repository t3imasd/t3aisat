...
class CameraScreenState extends State<CameraScreen>
  ...
  void initState() {
    super.initState();
    _initCamera();
    _loadLastCapturedAsset(); // Carga el último asset capturado
  ...
  // Method to load the last asset captured and update the thumbnail
  Future<void> _loadLastCapturedAsset() async {
    ...
    if (albums.isNotEmpty) {
      final List<AssetEntity> mediaFiles = await albums[0].getAssetListRange(
        start: 0,
        end: 50, // Get the last 50 multimedia files
      );

      // Itera through multimedia files to find the first valid asset
      for (final AssetEntity asset in mediaFiles) {
        ...
        if (isValid) {
          ...
          return; // Salir después de encontrar el asset válido más reciente
        }
      }

      // If no valid asset is found, establish _lastCapturedAsset in null
      setState(() {
  ...
  // Method for updating Thumbnail when it changes _lastCapturedAsset
  Future<void> _updateLastCapturedThumbnail() async {
    if (_lastCapturedAsset != null) {
      final thumbnail = await _lastCapturedAsset!.thumbnailDataWithSize(ThumbnailSize(100, 100));
      setState(() {
